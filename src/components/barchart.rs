...
impl<'a> Widget for BarChart<'a> {
    fn render(mut self, area: Rect, buf: &mut Buffer) {
        ...
        let max = self.max.unwrap_or_else(|| {
            self.data.iter().map(|t| t.1).max().unwrap_or_default()
        });
        let max_index = min(
        ...
        let mut data = self
            ...
            .map(|&(l, v, s)| {
                (
                    l,
                    v * u64::from(chart_area.height - 1) * 8
                        / std::cmp::max(max, 1),
                    s,
                )
            })
            .collect::<Vec<(&str, u64, Option<Style>)>>();
        ...
        for (i, &(label, value, _)) in
            self.data.iter().take(max_index).enumerate()
        {
            if value != 0 {
